...
Installing `gulp-util` from the `conda-forge` channel can be achieved by adding `conda-forge` to your channels with:

```
conda config --add channels conda-forge
```

Once the `conda-forge` channel has been enabled, `gulp-util` can be installed with:

```
conda install gulp-util
```

It is possible to list all of the versions of `gulp-util` available on your platform with:

```
conda search gulp-util --channel conda-forge
```
...
Updating gulp-util-feedstock
============================

If you would like to improve the gulp-util recipe or build a new
...
Note that all branches in the conda-forge/gulp-util-feedstock are
